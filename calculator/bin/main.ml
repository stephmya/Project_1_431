...
open Owl
open Owl.Mat
open Str  (* Import Str for regular expressions *)
...
(* Helper function to perform basic operations on matrices *)
let matrix_operations mat1 mat2 operator =
  (* Helper function to check if any element in the matrix is zero *)
  let is_zero_matrix mat =
    Owl.Mat.exists (fun x -> x = 0.0) mat
  in
  match operator with
  | "+" -> Owl.Mat.add mat1 mat2
  | "-" -> Owl.Mat.sub mat1 mat2
  | "*" -> Owl.Mat.mul mat1 mat2
  | "/" -> 
      if is_zero_matrix mat2 then
        failwith "Division by zero"
      else
        Owl.Mat.div mat1 mat2
  | _ -> failwith "Unknown operator"

(* Function to evaluate a basic arithmetic expression with matrices *)
let evaluate_expression expr =
  try
    ...
    if Str.string_match re expr 0 then
      ...
      if expr_len >= 4 && String.sub expr 0 4 = "sin(" then
        let operand = String.sub expr 4 (expr_len - 5) |> float_of_string in
        let mat = Owl.Mat.of_array [|operand|] 1 1 in
        let result = Owl.Mat.sin mat in
        Owl.Mat.to_array result
        |> Array.fold_left (fun acc x -> acc ^ " " ^ string_of_float x) ""
      else if expr_len >= 4 && String.sub expr 0 4 = "cos(" then
        let operand = String.sub expr 4 (expr_len - 5) |> float_of_string in
        let mat = Owl.Mat.of_array [|operand|] 1 1 in
        let result = Owl.Mat.cos mat in
        Owl.Mat.to_array result
        |> Array.fold_left (fun acc x -> acc ^ " " ^ string_of_float x) ""
      else
        "Invalid expression"
